<template>
  <div>
    <van-nav-bar title="跟师心得" class="header" fixed left-arrow @click-left="onClickLeft"></van-nav-bar>
    <div class="layout_content" ref="layoutContent" @scroll="scroll()">
      <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
        <van-tabs v-model="active" :offset-top="40" sticky swipeable title-active-color="#1a7fe9" color="#5fa4ec" :line-height="2" @change="change">
          <div v-for="(item,index) in tabs">
            <van-tab :title="item.title">
              <van-list v-model="item.loading" :finished="item.finished" :error.sync="item.error" error-text="请求失败，点击重新加载" :finished-text="item.finishedText" immediate-check :offset="50" @load="onLoad()">
                <!-- <ViewList :listObj="item.listObj" :listData="item.listData"> </ViewList> -->
                <Commonality :listData="item.listData" :active="active" :listObj="item.listObj" :isclinical="false">
                </Commonality>
              </van-list>
            </van-tab>
          </div>
...
// import { ToReviewlist ,PendingReviewlist} from "@/mockjs/apimock";
export default {
  ...
  methods: {
    ...
    queryData(currentPage) {
      ...
      this.utils.ajax({
        method: "post",
        data: params,
        url: this.api.supervisorNoteList,
        success: data => {
          this.isLoading = false;
          if (data.content.length) {
            data.content.forEach(item => {
              item.traineeMajorCodeText = this.utils.getValueByKey(
                this.$store.state.majors,
                item.traineeMajorCode
              );
              item.noteType = this.utils.getValueByKey(
                this.$store.state.noteType,
                item.noteType
              );
            });
          } else {
